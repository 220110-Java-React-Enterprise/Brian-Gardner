...
import CRUD_Repo.AccountTransactionRepoCRUD;
...
//Menu to allow customers to withdraw money from their accounts
public class WithdrawalMenu extends View {
    ...
    public void renderView() {
        //AccountTransactionRepo objects used to create new withdrawal
        AccountTransactionRepo accountTransactionRepo = new AccountTransactionRepo();

        //Variable to store number of steps left in withdrawal process
        int steps = MAX_STEPS;
        ...
        while (steps > 0) {
            while (steps == MAX_STEPS) {
                System.out.println(DataStore.getAccountModel() + "Withdraw from current bank account or another? (enter 1-2; 0 to exit)\n" +
            ...
            while (steps == MAX_STEPS - 1) {
                System.out.println("Account balance: " + accountModel.getBalanceString());
                System.out.println("How much would you like to withdraw? (enter x to exit)");
                strInput = viewManager.getScanner().nextLine();
                ...
                if (dblInput < .01) {
                    System.out.println("Error: cannot withdraw less than $0.01");
                    continue;
                }

                //Test if input would withdraw more than account balance
                if (dblInput > accountModel.getBalance()) {
                    System.out.println("Error: cannot withdraw more than available in account balance.");
                    continue;
                }
        ...
        //Exit WithdrawalMenu, using steps variable to signal different codes for next menu
        ...
        switch (steps) {
            ...
            case -1: viewManager.navigate("UI.TransactionMenus.TransactionMenu");
                break;
            //Code for successfully creating withdrawal in SQL database
